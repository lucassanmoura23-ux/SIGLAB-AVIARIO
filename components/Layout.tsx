...
import { LayoutDashboard, List, FileText, PlusCircle, LineChart, ClipboardCheck } from 'lucide-react';
...
const NavItem = ({ 
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center space-x-3 px-3 py-2 rounded-md transition-colors mb-0.5 ${
        isActive 
...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, setView }) => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      {/* Sidebar - Desktop */}
      <aside className="hidden md:flex flex-col w-60 bg-white border-r border-gray-200 fixed h-full z-10 shadow-lg">
        <div className="p-3 border-b border-gray-100 flex flex-col items-center text-center bg-white">
          
          {/* Logo Container - Super Compacto */}
          <div className="w-20 h-20 flex items-center justify-center mb-1 transition-transform duration-300 hover:scale-105">
            {!logoError ? (
              <img 
                src="logo.png" 
                alt="SIGLAB Logo" 
              ...
              <div className="w-16 h-16 rounded-full bg-gradient-to-b from-blue-900 to-blue-950 flex flex-col items-center justify-center mb-1 shadow-md border-2 border-blue-500">
                <div className="text-white font-black text-lg tracking-tighter">SIG</div>
              </div>
            )}
          </div>

          <h1 className="text-base font-bold text-blue-900 tracking-tight leading-none">
            Gestão Aviária
          </h1>
          <p className="text-[10px] text-gray-400 font-medium mt-0.5">Controle de Produção</p>
        </div>
        ...
        <nav className="flex-1 px-2 py-2 overflow-y-auto">
          ...
          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 mt-3 px-2">Dados</div>
          <NavItem 
            view={ViewState.LIST} 
            current={currentView} 
            label="Registros" 
          ...
          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 mt-3 px-2">Qualidade</div>
          <NavItem 
          ...
          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 mt-3 px-2">Análise</div>
          <NavItem 
        ...
        <div className="p-2 bg-gray-50 border-t border-gray-200 text-[10px] text-center">
          <p className="font-bold text-[#1e3a8a]">SIGLAB v1.0.0</p>
          <p className="text-gray-400 text-[9px]">Todos os direitos reservados</p>
        </div>
      </aside>
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="h-9 w-9 flex items-center justify-center">
               {!logoError ? (
                 ...
                 <div className="h-8 w-8 rounded-full bg-blue-900 flex items-center justify-center text-white font-bold text-[10px] border border-blue-700">
                    SIG
                 </div>
               )}
            </div>
            <div className="leading-tight">
                ...
                <p className="text-[10px] text-gray-500">Gestão Aviária</p>
            </div>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 md:ml-60 p-4 md:p-6 pt-16 md:pt-6 overflow-x-hidden min-h-screen">
        {children}
      </main>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around p-1 z-20 shadow-[0_-1px_3px_rgba(0,0,0,0.1)] pb-2">
         ...
         <button onClick={() => setView(ViewState.CHARACTERIZATION_LIST)} className={`p-2 rounded flex flex-col items-center ${currentView === ViewState.CHARACTERIZATION_LIST ? 'text-blue-600' : 'text-gray-400'}`}>
            ...
            <span className="text-[9px] mt-0.5">Caract.</span>
         </button>
